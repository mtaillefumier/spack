...
class Deepmd(PythonExtension, CudaPackage, ROCmPackage, CMakePackage):
    ...
    version(
        "3.0.0",
        sha256="4df1091ce90dbea87734a20c6d826b8cbe80ac44646cb592c2e8586be319023c",
    )
    version(
        "3.0.0b0",
        sha256="44e5a6255f7890f4b9f1cc5e1525380c1d1e41dd75ac9e738d5b81a728759bba",
    )
    version(
        "3.0.0a0",
        sha256="36e9dc1b18313139b1e9482e06ec214e15a87ee74f5f9d565b8719998e088920",
    )
    version(
        "2.2.11",
    ...
    variant(
        "tensorflow",
        default=True,
        description="Enable tensorflow support (original ML backend)",
    )
    variant(
        "pytorch", default=False, description="Enable pytorch support (starting v3.0.0)",
        when="@3.0:"
    )
    ...
    with when("+cuda"):
        ...
        for target in CudaPackage.cuda_arch_values:
            ...
            depends_on(f"py-horovod+cuda tensor_ops=nccl cuda_arch={target}", when=f"+horovod cuda_arch={target}")
            depends_on(f"py-tensorflow+cuda+nccl+mpi cuda_arch={target}", when=f"+tensorflow cuda_arch={target}")
    ...
    with when("+rocm"):
        ...
        for target in ROCmPackage.amdgpu_targets:
            depends_on(f"py-tensorflow@2.16:+rocm+mpi amdgpu_target={target}", when=f"+tensorflow amdgpu_target={target}")
            depends_on(f"py-torch+rocm amdgpu_target={target}", when=f"+pytorch amdgpu_target={target}")
    ...
    def setup_build_environment(self, env):
        ...
        if "+tensorflow +fp64" in self.spec:
    ...
    def install_python(self):
        args = std_pip_args + ["--prefix=" + self.prefix, self.stage.source_path]
        pip(*args)
